...
- [TrafficGuard](#trafficguard)
  ...
  - [О проекте](#о-проекте)
    ...
    - [Способы использования](#способы-использования)
      - [1. Базовая защита сервера](#1-базовая-защита-сервера)
      - [2. Защита с мониторингом](#2-защита-с-мониторингом)
      - [3. Комплексная защита](#3-комплексная-защита)
      - [4. Собственные списки блокировки](#4-собственные-списки-блокировки)
    - [Правовой статус и легальность](#правовой-статус-и-легальность)
      - [Что это такое юридически?](#что-это-такое-юридически)
      - [Правовая квалификация](#правовая-квалификация)
      - [Что делает утилита с точки зрения закона?](#что-делает-утилита-с-точки-зрения-закона)
      - [Правовые основания (Российская Федерация)](#правовые-основания-российская-федерация)
      - [Может ли быть привлечение к ответственности?](#может-ли-быть-привлечение-к-ответственности)
      - [Для государственных органов](#для-государственных-органов)
      - [Рекомендации](#рекомендации)
  - [Быстрый старт](#быстрый-старт)
  - [Установка](#установка)
    - [Автоматическая установка](#автоматическая-установка)
    - [Ручная установка](#ручная-установка)
    - [NixOS модуль (декларативно, через nixos-rebuild)](#nixos-модуль-декларативно-через-nixos-rebuild)
...
### NixOS модуль (декларативно, через nixos-rebuild)

Флейк экспортирует `nixosModules.default`. Модуль принимает список URL (аналог нескольких `-u`) и:
- на `nixos-rebuild switch/test` скачивает списки и применяет правила;
- на загрузке системы только восстанавливает правила из кэша (`/var/lib/traffic-guard/ipset.conf`) без повторной загрузки URL.
- при ошибке refresh на `switch/test` завершает активацию ошибкой (без silent fallback).

Пример подключения в вашем `flake.nix`:

```nix
{
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    traffic-guard.url = "github:LocalT0aster/nix-traffic-guard";
  };

  outputs = { self, nixpkgs, traffic-guard, ... }: {
    nixosConfigurations.my-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        traffic-guard.nixosModules.default
        ({ ... }: {
          services.traffic-guard = {
            enable = true;
            urls = [
...
Ограничение: модуль использует `iptables/ipset` и сейчас ожидает `networking.nftables.enable = false`.
